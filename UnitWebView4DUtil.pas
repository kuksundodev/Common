...
const
  MSG_LISTNER_4_BTN_CLICK_ON_MAIN_FRAME =
    ...
    '  console.log("message : window.addEventListener => " + event.data); ' +
    '  let data = event.data; '+
    '  data = JSON.parse(data); '+

    '  console.log("message : JSON.parse(data) => " + data.type); ' +

        // 버튼 클릭 이벤트 처리
    '    if (data.type === "iframe_button_click") { '+
  ...
  ADD_LISTNER_4_BTN_CLICK_ON_IFRAME =
    '(function () { '+
    '  var btn = document.getElementById(%s); '+  //btn_saved
    '  if (!btn) return %s + " not Found"; '+
//    '  else return "Button Found"; '+
    ...
    '  console.log("Add Listener Click : " + btn.id); ' +
    '})(); ';
...
type
  TWebView4DUtil = class
    ...
    class procedure ClickInputByIdFromIFrame(ABrowser: TWVBrowser; AFrame: TCoreWebView2Frame; const ASelector: string); static;
    ...
    class procedure GetElementRectUsingWebMsgFromIFrameList(ABrowser: TWVBrowser; AFrameList: TObjectList<TCoreWebView2Frame>; const ASelector: string; const AFrameIdx: integer=-1); static;
    class procedure GetElementRectUsingWebMsgFromIFrame(ABrowser: TWVBrowser; AFrame: TCoreWebView2Frame; const ASelector: string); static;
  end;
...
class procedure TWebView4DUtil.ClickInputByIdFromIFrame(ABrowser: TWVBrowser;
...
begin
  JSCode :=
//    ' (function () {                                                        ' +
//    '     var el = document.getElementById("' + ASelector + '");            ' +
//    '     if (el) {                                                          ' +
//    '         el.focus();                                                    ' +
//    '         el.click();                                                    ' +
...
//    ' })();                                                                  ' ;
    ...
    ' (function () {                                                                       ' +
    '     var edits = document.querySelectorAll(''input[type="text"]'');                   ' +
    '                                                                                      ' +
    '     for (var i = 0; i < edits.length; i++) {                                         ' +
    '         if (edits[i].id === "' + ASelector + '") {                                   ' +
    '            edits[i].focus();                                                          ' +  // 커서 이동
//    '            edits[i].click();                                                          ' +  // 클릭 이벤트
//    '            edits[i].value = '';                                                          ' +  // 클릭 이벤트
    '            console.log(                                                               ' +
    '             i,                                                                        ' +
    '             "id=", edits[i].id,                                                       ' +
    '             "value=", edits[i].value,                                                 ' +
    '             "class=", edits[i].className                                              ' +
    '            );                                                                         ' +
    '            return edits[i].id;                                                        ' +
    '         }                                                                             ' +
    '     }                                                                                 ' +
  ...
  AFrame.ExecuteScript(JSCode, CLICK_INPUT_EVENT_ID, ABrowser);
end;

class procedure TWebView4DUtil.ClickInputByIdFromIFrameList(
...
begin
  ...
  begin
    if AFrameIdx < AFrameList.Count then
      TWebView4DUtil.ClickInputByIdFromIFrame(ABrowser, AFrameList[AFrameIdx], ASelector)
  end
  else
  begin
    for i := 0 to AFrameList.Count - 1 do
    begin
      TWebView4DUtil.ClickInputByIdFromIFrame(ABrowser, AFrameList[i], ASelector);
    end;
  end;
end;
...
var
  ...
  i: integer;
begin
  JSCode := Format(ADD_LISTNER_4_BTN_CLICK_ON_IFRAME, [QuotedStr(ASelector), QuotedStr(ASelector)]);
//    '(function () { '+
...
class procedure TWebView4DUtil.GetElementRectUsingWebMsgFromIFrame(
  ABrowser: TWVBrowser; AFrame: TCoreWebView2Frame; const ASelector: string);
var
  JSCode: string;
begin
  JSCode :=
    ...
    '     for (var i = 0; i < edits.length; i++) {                                         ' +
    '         if (edits[i].id === "' + ASelector + '") {                                   ' +
    '           const rect = edits[i].getBoundingClientRect();    ' +
                // 요소의 정중앙 좌표 계산
  ...
  AFrame.ExecuteScript(JSCode, GET_RECT_EVENT_ID, ABrowser);

end;

class procedure TWebView4DUtil.GetElementRectUsingWebMsgFromIFrameList(
  ABrowser: TWVBrowser; AFrameList: TObjectList<TCoreWebView2Frame>;
  const ASelector: string; const AFrameIdx: integer);
var
  i: integer;
begin
  ...
  begin
    if AFrameIdx < AFrameList.Count then
      TWebView4DUtil.GetElementRectUsingWebMsgFromIFrame(ABrowser, AFrameList[AFrameIdx], ASelector)
  end
  else
  begin
    for i := 0 to AFrameList.Count - 1 do
    begin
      TWebView4DUtil.GetElementRectUsingWebMsgFromIFrame(ABrowser, AFrameList[i], ASelector);
    end;
  end;
end;
