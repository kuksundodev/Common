...
function RemovePrefix(const AString, APrefix: string): string;
begin
  if AString.StartsWith(APrefix) then
    Result := Copy(AString, Length(APrefix) + 1, MaxInt) // 접두사 뒤부터 끝까지
  else
    Result := AString; // 접두사가 없으면 원래 문자열 반환
end;
...
var
  LCleanStr: string;
begin
  // 1. 모든 콤마(,)를 빈 문자열('')로 대체하여 제거
  LCleanStr := StringReplace(AStr, ',', '', [rfReplaceAll]);

  // 2. 숫자로 변환 (빈 문자열일 경우를 대비해 TryStrToInt 사용 권장)
  if not TryStrToInt(LCleanStr, Result) then
    Result := 0; // 변환 실패 시 기본값end;
end;
...
var
  LPosSub: integer;
begin
  LPosSub := PosRev(ASubStr, AOrigStr);
...
begin
  ...
  // AText가 APrefix보다 짧으면 false
  if Length(AText) < LPrefixLen then
    Exit(False);

  // 앞부분을 잘라서 SameText로 비교 (대/소문자 무시)
  Result := SameText(Copy(AText, 1, LPrefixLen), APrefix);
end;
