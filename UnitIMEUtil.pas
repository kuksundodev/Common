...
uses  Windows, SysUtils, Winapi.imm, Controls;

type
  THanType = (htHangul, htHanja, htOther);
  TIMEState = (imeUnknown, imeEnglish, imeKorean);
...
procedure SetIMEKorean(AControl: TWinControl);
procedure SetIMEEnglish(AControl: TWinControl);
function GetIMEState(AControl: TWinControl): TIMEState;
...
begin
  ...
  if not Assigned(AControl) then Exit;
  if not AControl.Focused then Exit;

  IMC := ImmGetContext(AControl.Handle);
  if IMC = 0 then Exit;

  try
    if ImmGetConversionStatus(IMC, Conv, Sent) then
    begin
      if (Conv and IME_CMODE_NATIVE) <> 0 then
        Result := imeKorean
      else
        Result := imeEnglish;
    end;
