...
procedure SendKeysUnicode(const S: string);
procedure ExcuteKeyDown(AVKeyCode: integer);
procedure ExcuteKeyUp(AVKeyCode: integer);
function AppActivate(WindowName : PChar) : boolean;
function AppActivateWithHandle(WindowHandle : THandle) : boolean;
function GetKeyStr4SendKey(key: String): string;
function GetKeyStr4SendKey2(key: String): string;
function GetSpecialkeyNameFromVKCode(AVirtualKey: Byte): string;
function GetKeyNameFromVKCode(AVirtualKey: Byte; AUseKeyState: Boolean=True): string;
function GetNormalkeyNameFromVKCode(AVirtualKey: Byte; AUseKeyState: Boolean=True): string;
...
const
  ...
  (
//   (Name:'ALT';             VKey:VK_MENU),
//   (Name:'ALT';             VKey:VK_LMENU),
//   (Name:'ALT';             VKey:VK_RMENU),
...
//   (Name:'CONTROL';         VKey:VK_CONTROL),
//   (Name:'CONTROL';         VKey:VK_LCONTROL),
//   (Name:'CONTROL';         VKey:VK_RCONTROL),
...
//   (Name:'SHIFT';           VKey:VK_SHIFT),
//   (Name:'SHIFT';           VKey:VK_LSHIFT),
//   (Name:'SHIFT';           VKey:VK_RSHIFT),
    ...
    '~' : begin
      ...
      begin
        ...
        begin
          ...
          SendKeysUnicode(SendKeysString[I]);
...
begin
  ...
  begin
    ...
    if Pos('SHIFT', LStr) <> 0 then
      Result := '+';
    if Pos('CONTROL', LStr) <> 0 then
      Result := Result + '^';
    if Pos('ALT', LStr) <> 0 then
      Result := Result + '%';
  end;

//  if key = 'BS' then
//    Result := Result + '{BS}'
//  else if key = 'BREAK' then
//    Result := Result + '{Break}'
//  else if key = 'CAPS' then
//    Result := Result + '{CapsLock}'
...
//    Result := Result + '{Home}'
//  else if key = 'INSERTE' then
//    Result := Result + '{Insert}'
//  else if key = 'LEFT' then
//    Result := Result + '{Left}'
//  else if key = 'NUMLOCK' then
//    Result := Result + '{NumLock}'
//  else if key = 'PAGEDOWN' then
//    Result := Result + '{PgDn}'
//  else if key = 'PAGEUP' then
//    Result := Result + '{PgUp}'
//  else if key = 'PRTSC' then
//    Result := Result + '{PrtSc}'
//  else if key = 'RIGHT' then
//    Result := Result + '{Right}'
//  else if key = 'SCROLLLOCK' then
//    Result := Result + '{ScrollLock}'
...
//  else if key = 'F9' then
//    Result := Result + '{F9}'
//  else if key = 'F10' then
//    Result := Result + '{F10}'
//  else if key = 'F11' then
//    Result := Result + '{F11}'
//  else if key = 'F12' then
//    Result := Result + '{F12}'
//  else
    Result := Result + key;
end;
...
begin
  ...
  for i := 1 to MaxSendKeyRecs do
  begin
    if SendKeyRecs[i].VKey = AVirtualKey then
    begin
      Result := '{' + SendKeyRecs[i].Name + '}';
      Break;
    end;
  end;
end;

function GetKeyNameFromVKCode(AVirtualKey: Byte; AUseKeyState: Boolean): string;
//var
//  LSpecialKey: string;
begin
  Result := GetSpecialkeyNameFromVKCode(AVirtualKey);

  if Result = '' then
    Result := GetNormalkeyNameFromVKCode(AVirtualKey, AUseKeyState);
end;

function GetNormalkeyNameFromVKCode(AVirtualKey: Byte; AUseKeyState: Boolean): string;
var
  WBuff: array[0..255] of Char;
  KeyboardState: TKeyboardState;
  UResult: integer;
begin
  Result := '';

  if AUseKeyState then
    GetKeyBoardState(KeyboardState);
//  GetAsyncKeyState(KeyboardState);
  ZeroMemory(@WBuff[0], SizeOf(WBuff));
  UResult := ToUnicode(AVirtualKey, MapVirtualKey(AVirtualKey,0), KeyboardState, WBuff, Length(WBuff), 0);
//  UResult := ToAscii(AVirtualKey, MapVirtualKey(AVirtualKey,2), KeyboardState, WBuff, Length(WBuff));
